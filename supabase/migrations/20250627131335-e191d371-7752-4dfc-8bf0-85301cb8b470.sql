...
-- Fix the get_companion_time_slots function to show BOTH default and specific slots
...
DECLARE
    ...
    has_unavailable_slots boolean := false;
BEGIN
    ...
    -- Check if companion has available slots for this date
    SELECT EXISTS(
    ...
    -- Check if companion has unavailable slots for this date
    SELECT EXISTS(
        SELECT 1 FROM public.companion_availability ca
        WHERE ca.companion_id = _companion_id AND ca.date = _date AND ca.is_available = false
    ) INTO has_unavailable_slots;
    
    -- If no specific slots at all, return default slots
    ...
    -- If has specific slots, show COMBINED: default slots + specific available slots - unavailable periods
    ELSE
        RETURN QUERY
        ...
        -- Get default slots that don't overlap with unavailable periods
        filtered_default_slots AS (
            SELECT 
                'combined_default'::text as slot_type,
        ...
        -- UNION both default (filtered) and specific slots
        SELECT * FROM filtered_default_slots
        UNION ALL
