-- Update time slots from 9 AM - 5 PM to 8 AM - 8 PM
...
BEGIN
    ...
    -- If NO specific slots at all, return ONLY default slots (8 AM to 8 PM)
    IF NOT has_specific_slots THEN
        RETURN QUERY
        WITH RECURSIVE time_slots AS (
            SELECT 
                '08:00:00'::time as slot_start_time,
                '08:30:00'::time as slot_end_time
            UNION ALL
            SELECT 
                (slot_start_time + interval '30 minutes')::time,
                (slot_end_time + interval '30 minutes')::time
            FROM time_slots
            WHERE slot_start_time < '19:30:00'::time
        )
        SELECT 
