-- Update check_booking_conflict function to use 8 AM - 8 PM range
...
BEGIN
    ...
    IF has_specific_slots THEN
        ...
        -- Check if booking is within default hours (8 AM to 8 PM) and doesn't overlap with unavailable periods
        IF _start_time >= '08:00:00'::time AND booking_end_time <= '20:00:00'::time THEN
    ...
    -- No specific slots, so only check default hours (8 AM to 8 PM)
    IF _start_time < '08:00:00'::time OR booking_end_time > '20:00:00'::time THEN
        RETURN true;
    END IF;
